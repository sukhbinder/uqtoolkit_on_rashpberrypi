...
if foundMPL:
    ...
    rects1 = ax.bar(ind, SiT1mn, width/2.0, color='r', yerr=SiT1std,error_kw=dict(linewidth=3, color='b',capsize=5) )
    SiT2mn  = npy.array([npy.average(runSiT_2[:,i]) for i in range(ndim)])
    SiT2std = npy.array([npy.std(runSiT_2[:,i]) for i in range(ndim)])
    rects2 = ax.bar(ind+width/2.0, SiT2mn, width/2.0, color='y', yerr=SiT2std,error_kw=dict(linewidth=3, color='b',capsize=5) )
    plt.plot(ind+width/2.0,SiTmath,'o',ms=8,mfc='k')
    ax.set_xlim([0,ndim])
    ax.set_ylim([0,0.55])
