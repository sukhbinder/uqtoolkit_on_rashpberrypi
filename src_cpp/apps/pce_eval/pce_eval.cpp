...
/// \brief Evaluates a PC given dimensionality, order, and coefficients file
void fEval_PC(Array2D<double>& xdata, Array1D<double>& ydata, int pcdim, int pcord, char* param_file, char* pcType, double alpha, double beta);
/// \brief Evaluates a PC given multiindex file and coefficients file
void fEval_PCmi(Array2D<double>& xdata, Array1D<double>& ydata, char* param_file, char* pcType, char* miFile, double alpha, double beta);
...
/*----------------------------------------------------------------------------*/ 
  ...
  if (string(fcn_type) =="PC")
    fEval_PC(xdata,ydata, xdata.YSize(), pcord, param_file, str1,a, b);
  else if (string(fcn_type) =="PC_mi")
    fEval_PCmi(xdata,ydata, param_file, str1,str2,a, b);
  else if (string(fcn_type) =="PCmap")
    fEval_PCmap(xdata,ydata,string(str1),a,b,string(str2),c,d);
  else
    throw Tantrum("f_eval.cpp::Function type is not recognized");
...
void fEval_PC(Array2D<double>& xdata, Array1D<double>& ydata, int ndim, int nord, char* param_file, char* pcType, double alpha, double beta)
{
  ...
  read_datafile_1d(c_k,param_file);

  /// Check that the inputs are in the domain
/*
 for(int i=0;i<ns;i++){
    for(int id=0;id<(int)xdata.YSize();id++){
      if ( !currPCModel.IsInDomain(xdata(i,id)) )
        throw Tantrum("fEval_PC() : PC argument outside bounds!");
    }
  }
*/
...
/*
  for(int i=0;i<ns;i++){
    for(int id=0;id<(int)xdata.YSize();id++){
      if ( !currPCModel.IsInDomain(xdata(i,id)) )
        throw Tantrum("fEval_PCmi() : PC argument outside bounds!");
    }
  }
*/
  /// Evaluate the PC expansion
...
void fEval_PCmap(Array2D<double>& xdata, Array1D<double>& ydata,  string pcIn, double a,double b, string pcOut, double c,double d)
{
  ...
  /// Compute the PC map using function in uqtktools.cpp
  for(int i=0;i<ns;i++)
