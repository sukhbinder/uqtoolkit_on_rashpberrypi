...
int usage(){
  printf("usage: generate_quad [-h] [-r] [-d<ndim>] [-g<quadType>] [-x<fsType>] [-p<param>] [-a<alpha>] [-b<beta>] [-s<domain_file>]\n");
...
int main (int argc, char *argv[]) 
{
  ...
  while ((c=getopt(argc,(char **)argv,"hrd:g:x:p:a:b:s:"))!=-1){
    switch (c) {
    case 'h':
      usage();
      break;
...
/*----------------------------------------------------------------------------*/ 
  ...
  if (sflag && string(quadType)!="CC" && string(quadType)!="CCO" && string(quadType)!="NC" && string(quadType)!="NCO" && string(quadType)!="LU" && string(quadType)!="JB") 
